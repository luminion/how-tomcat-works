...
/**
 * 简单的上下文路径context容器
 * 初始化成员变量时，创建了3个hashmap，分别用于存放子容器，存放映射关系，存放映射器
 * 初始化成员变量时，会创建一个SimplePipeline管道
 * 创建时，构造器会创建一个SimpleContextValve基础阀并设置为管道的基础阀
 *
 * 当连接器与容器关联后，
 * 连接器收到请求后，会调用容器的invoke方法，
 * 容器的invoke方法调用管道的invoke方法，
 * 管道通过阀的控制依次调用所有普通阀的invokeNext()方法和基础阀的invoke()方法
 * 基础阀的invoke()方法调动servlet的service()方法，处理请求
 *
 */
