...
public class HttpRequest implements HttpServletRequest {
   ...
   * synchronized.
   * 此请求的解析参数。
   * 仅当通过getParameter()方法调用系列之一请求参数信息时，才会填充此参数。
   * 键是参数名称，而值是此参数的值的字符串数组。
   *
   * 一旦特定请求的参数被解析并存储在此处，它们就不会被修改。
   * 因此，对参数的应用程序级访问不需要同步。
   */
   ...
   * 参数都不会被解析(提高性能)，
   * 仅当调用该4个方法时，
   * 该4个方法会调用parseParameter()方法解析参数
   * 当该值为true时，parseParameter()会直接返回
   *
   */
  ...
  protected void parseParameters() {
    ...
    // 将请求字符串传递给RequestUtil，让其解析请求中的参数
